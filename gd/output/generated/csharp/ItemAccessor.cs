...
public static class ItemAccessor
{
    ...
    private static void _EnsureLoaded()
    {
        if (_mainIndex == null)
            throw new InvalidOperationException(
                "ItemAccessor.Preload() must be called before accessing data.");
    }
    ...
    private static int _GetRowIndex(int id)
    {
        ...
        if (!_mainIndex.TryGetValue(id, out int idx))
            throw new KeyNotFoundException($"ItemAccessor: id {id} not found.");
        return idx;
    }
    ...
    public static float GetPrice(int id)
    {
        int idx = _GetRowIndex(id);
        var root = ItemTable.GetRootAsItemTable(_bb);
        return root.Items(idx).Value.Price;
    }

    public static byte GetRarity(int id)
    {
        int idx = _GetRowIndex(id);
        var root = ItemTable.GetRootAsItemTable(_bb);
        return (byte)root.Items(idx).Value.Rarity;
    }
    ...
    public static DropRange GetDropRange(int id)
    {
        int idx = _GetRowIndex(id);
        var root = ItemTable.GetRootAsItemTable(_bb);
        return root.Items(idx).Value.DropRange.Value;
    }

    public static int[] GetTags(int id)
    {
        int idx = _GetRowIndex(id);
        var root = ItemTable.GetRootAsItemTable(_bb);
        var row = root.Items(idx).Value;
        int len = row.TagsLength;
        var arr = new int[len];
        for (int i = 0; i < len; i++)
            arr[i] = row.Tags(i);
        return arr;
