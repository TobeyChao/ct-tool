...
def _generate_struct_table(field: FieldDef, enums: list[str]) -> str:
    ...
    # 类型名加 Struct 后缀，避免与字段名同名（flatc 拒绝字段名 == 类型名）
    name = f"{field.name}Struct"
    lines = [f"table {name} {{"]
