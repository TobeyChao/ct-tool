...
export async function mount(container) {
  ...
  state.query = state.query || localStorage.getItem("ct-v4-filter") || "";
  ...
  state.collapsedGroups = state.collapsedGroups || readJsonPreference("ct-v4-resource-groups", {});
  state.recentResources = state.recentResources || readJsonPreference("ct-v4-recent-resources", []);
  ...
  const savedResourceW = localStorage.getItem("ct-v4-resource-w-wide");
  const savedSideW = localStorage.getItem("ct-v4-side-w-wide");
  ...
  function renderList() {
    ...
    list.querySelectorAll(".ct-group-toggle").forEach((toggle) => {
      toggle.addEventListener("click", () => {
        ...
        writeJsonPreference("ct-v4-resource-groups", state.collapsedGroups);
        renderList();
      });
    });
  }
  ...
  function openResource(name) {
    ...
    writeJsonPreference("ct-v4-recent-resources", state.recentResources);
    renderList();
    renderEditor();
    renderInspector();
  ...
  filterInput.addEventListener("input", () => {
    ...
    try { localStorage.setItem("ct-v4-filter", state.query); } catch (e) { /* ignore */ }
    renderList();
  });
  ...
  function wireResize() {
    ...
    if (resourcePane && editor) {
      ...
      handle.addEventListener("mousedown", startDrag(
        ...
        try { localStorage.setItem("ct-v4-resource-w-wide", String(w)); } catch (err) { /* ignore */ }
      }));
    }
    if (sidePane && editor) {
      const handle = document.createElement("div");
      ...
      handle.addEventListener("mousedown", startDrag(
        ...
        try { localStorage.setItem("ct-v4-side-w-wide", String(w)); } catch (err) { /* ignore */ }
      }));
    }
  }
}
