...
from ct.app import canonical_export as export
...
def _assert_forced_equal(root):
    ...
    result = export.run_canonical_export(root, forced=True)
...
def test_warm_export_skips_generators_and_preserves_mtimes(project, monkeypatch):
    export.run_canonical_export(project)
    ...
    result = export.run_canonical_export(project)
...
def test_translation_only_rebuilds_affected_language(project):
    ...
    assert export.run_canonical_export(project)["cache_misses"] == 0
...
def test_full_export_after_filtered_export_restores_complete_bundle(project):
    export.run_canonical_export(project, table_filter="Item", lang_filter="en")
    export.run_canonical_export(project)
    _assert_forced_equal(project)
...
def test_full_export_reclaims_obsolete_cache_entries(project):
    ...
    export.run_canonical_export(project, table_filter="Item")
