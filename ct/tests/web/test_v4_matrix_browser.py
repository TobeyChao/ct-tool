"""v4 shell viewport x zoom screenshot matrix (13.3).
...
MATRIX = CT_ROOT / "tests/fixtures/web/schema_workbench_v4_matrix.json"
...
def v4_matrix_url(tmp_path_factory) -> Iterator[str]:
    from _v4_helpers import build_v4_project

    workspace = build_v4_project(
        tmp_path_factory.mktemp("v4matrix") / "workspace",
        schemas=[
...
def test_v4_shell_matrix(
    ...
    output_root = Path(os.environ.get("CT_V4_SCREENSHOT_DIR", str(tmp_path)))
    ...
    page.goto(v4_matrix_url + "#/export", wait_until="load")
    ...
    page.screenshot(path=str(output_root / f"v4-{w}x{h}-z{zoom}-{expected}.png"))
    context.close()
