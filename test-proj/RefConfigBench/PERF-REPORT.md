...
> 结论先行：**加载持平，稳态字符串访问 ct 快 3.2×，热路径标量/向量读取 ct 慢 3.5×–9.6×；
...
而单条非对齐 32 位加载只要 1 次。该函数位于**每一次**字段读、二分比较、向量长度读取的最内层。
...
### 3.2 生成器没有接线 reader 已实现的「偏移预取」快路径
