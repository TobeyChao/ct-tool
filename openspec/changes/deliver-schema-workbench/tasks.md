...
- [x] 1.3 Add Playwright to the Python test environment, drive the local Web Panel without a frontend build chain, define the  screenshot fixtures and viewport/zoom matrix, and verify CI/local baseline captures plus a packaged 720×460 launcher smoke capture.
...
- [x] 7.6 Route existing Schema read endpoints through WorkspaceSnapshot, remove direct writable endpoints after the  cutover, and verify production routes expose only one write protocol.
...
- [x] 13.6 After the architecture/cleanup gate in section 14 passes, enable the  production entry, remove the temporary feature gate and old writable Schema path, and verify a clean install plus packaged launcher workspace completes the same end-to-end flow.
