...
Recorded on 2026-09-02 before the v4 production entry was enabled.
...
- Workspace: `ct/tests/fixtures/repository_cutover_v4/workspace` inputs copied to
...
- Full local captures: `test-artifacts/web-v4-baseline/` (ignored generated
  output); CI uploads the same directory as the `web-v4-baseline` artifact.
- Retained representatives:
  - `panel-pre-v4-720x460-z100.png`
  - `panel-pre-v4-390x844-z150.png`

These are intentionally pre-v4 defect/reference captures, not approval goldens
for the final workbench. The matrix manifest at
`ct/tests/fixtures/web/schema_workbench_v4_matrix.json` remains the executable
fixture for later v4 screenshots and layout assertions.

## Final v4 workbench qualification
