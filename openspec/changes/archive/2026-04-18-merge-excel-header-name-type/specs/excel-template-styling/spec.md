...
### Requirement: type 行作为独立表头行渲染
**Reason**: 字段类型不再独立成行，已合并入字段名单元格内的富文本第二段；旧的"type 行 `D8F3DC` 浅绿底 + 深绿色斜体"配色规则不再适用。

**Migration**: 调用 `ct gen-template --all` 重新生成所有模板。reader 端不需要任何改动（只依赖 `schema.header_rows`）。
