...
`ct export` SHALL 执行 canonical 完整校验覆盖并默认增量复用有效解析/校验结果及生成产物，`--all` SHALL 绕过解析、校验和生成缓存，完整处理选中表及 ref 依赖并强制生成及写出所有选中产物。对外步骤 SHALL 保持 `解析校验 → JSON → Accessor → FBS → Bundle`，不新增 Deploy 步骤。导出 SHALL 不执行 i18n sync、不写 i18n/source、不调用 flatc。
...
- **WHEN** 用户执行 `ct export`
- **THEN** 全部表完成当前输入的校验覆盖（允许有效缓存复用），全部语言生成或复用 JSON/FBS/Accessor/Bundle，未变内容保留 mtime；依次输出导出完成和部署结果
